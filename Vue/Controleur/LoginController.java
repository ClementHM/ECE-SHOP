...
public class LoginController implements Initializable {
    ...
    public void connexionbouton(ActionEvent event){
        ...
        for (int i=0;i<cc.size();i++){
            if ((cc.get(i).getEmail().equals(tf_email.getText() ) )&& (cc.get(i).getPassword().equals(tf_password.getText()))){
                try {
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("bite.fxml"));
                    root = loader.load();
                    stage = (Stage)((Node)event.getSource()).getScene().getWindow();
                    scene = new Scene(root);
                    stage.setScene(scene);
